// ESERCIZIO 2-1: Random Walks discreti
// Calcolare la distanza media (come radice della media dei quadrati delle posizioni finali) di una serie di random walks discreti al variare del numero di passi effettuati
...
// Funzione che genera direzioni discrete
// Estraggo un numero casuale tra 0 e 5 e questo sarà l'indice estratto da un vettore di stringhe con i 6 versi possibili nelle 3 direzioni
std::string RandDirection(int rand) {
    std::vector<std::string> direzioni = {"UP", "DOWN", "RIGHT", "LEFT", "OUT", "IN"};
    return direzioni[rand];
}
...
// Definisco le variabili che mi servono per l'analisi
	...
	const int M = Ndati / P;	// Numero di random walk lunghi P passi
	const int N = 100; 			// Numero di blocchi
	const int L = M / N; 		// Dati per blocco

	// const double a = 1.0; 		// Lunghezza di un passo fissata a 1 dal metodo di generazione delle direzioni casuali
	
	std::vector<string> dir(Ndati); // Conterrà le direzioni casuali discrete

	// Genero le direzioni casuali discrete
	for(int i=0; i< Ndati; i++){
		dir[i] = RandDirection(static_cast<int>(rnd.Rannyu(0,6)));
	}
...
// Vector per la divisione a blocchi
	...
	std::vector<std::vector<double>> s(M, std::vector<double>(P)); // matrice che contiene M random Walk di P passi, ovvero a passo fissato contiene la distanza quadro raggiunta a quel passo per quello specifico RW. Ad esempio, s[k][:] è una lista di P distanze quadro riguardanti il k-esimo Random Walk
...
// prima calcolo le posizioni quadro finali raggiunte da ogni random walk a passo fissato
	...
	// Ogni simulazione parte dall'origine
	    ...
	    for (int p = 0; p < P; p++) { // ad ogni passo calcolo la distanza quadro raggiunta
	        int k = p + m * P;
	// la posizione al passo successivo si basa su quella raggiunta al passo precedente, sommo gli spostamenti nelle varie direzioni
		if (dir[k] == "RIGHT") 	{dir_x += 1;}
		if (dir[k] == "LEFT") 	{dir_x -= 1;}
		if (dir[k] == "UP") 	{dir_y += 1;}
		if (dir[k] == "DOWN") 	{dir_y -= 1;}
		if (dir[k] == "OUT") 	{dir_z += 1;}
		if (dir[k] == "IN") 	{dir_z -= 1;}
...
// Ora per ogni passo faccio la media a blocchi sugli M RW dei risultati ottenuti
	for (int p = 0; p < P; p++) {
	    for (int i = 0; i < N; i++) { // per ogni blocco
	        double sum = 0.0; // azzero il conteggio della distanza
	        for (int j = 0; j < L; j++) { // per ogni dato del blocco
	            int k = j + i * L;
	            sum += s[k][p];
	        }
  ...
  // Genero un file esterno in cui verranno salvate le medie cumulative e relativo errore della distanza raggiunta
	  ofstream out;
	  out.open("dati/Dati_es2-2-1.txt");

  // Controllo la corretta apertura del file
	  if (!out.is_open()) {
	    cerr << "Error: unable to open Dati_es2-2-1.txt" << endl;
	    exit(1);
	  }
	...
	// ora calcolo per ogni passo la media delle medie sui blocchi (delle distanze al quadrato), ovvero è come fare la media cumulativa e prendere il risultato ottenuto cumulando tutti i blocchi
